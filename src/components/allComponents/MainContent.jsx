...
function MainContent({ openShowDetails }) {
  // Accessing the state and dispatch function from StoreContext
  const { state, dispatch } = useContext(StoreContext);

  // useEffect hook to set up initial data for books when the component mounts
  useEffect(() => {
    // Adding an 'isFavorite' property to each book, initially set to false
    const booksWithFavorites = data.map((book) => ({
      ...book,
      isFavorite: false,
    }));
    // Dispatching action to set the books in the state
    dispatch({ type: "SET_BOOKS", payload: booksWithFavorites });
  }, [dispatch]); // This runs only once when the component is mounted

  // Function to toggle the 'favorite' status of a book
  const toggleFavorite = (id) => {
    dispatch({
      type: "TOGGLE_FAVORITE",
      payload: id, // Dispatching action to toggle the favorite status of the selected book
    });
  };
    ...
    <div className="relative grid items-center justify-between grid-cols-2 gap-4 p-3 overflow-x-auto border border-gray-700 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
      ...
      {booksToDisplay.map((book, idx) => {
        ...
        return (
          ...
          >
            {/* Book image, clicking on it opens the book details */}
            <img
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              {isInCart ? (
                <button className="relative flex items-center justify-center gap-2 px-4 py-2 text-white transition-all duration-300 transform rounded bg-slate-500 sm:w-36 md:w-36 hover:scale-105 hover:rotate-3d hover:shadow-lg active:scale-95">
                  <p className="px-2 text-sm font-semibold text-black border-r border-black whitespace-nowrap">
                    ${book.price}
                  </p>
                  <span className="text-sm font-semibold text-center text-black whitespace-nowrap">
                    Added
                  </span>
                </button>
              ) : (
                <button
                  ...
                  className="relative flex items-center justify-center gap-2 px-2 py-2 text-white transition-all transform rounded duration-400 sm:w-36 md:w-36 bg-gradient-to-r from-green-400 to-green-600 hover:scale-105 hover:rotate-4 hover:shadow-xl hover:shadow-green-500/50 active:scale-95"
                >
                  {/* Animated background for Add to Cart button on hover */}
                  ...
                  <p className="z-10 text-sm font-semibold text-white whitespace-nowrap">
                    Add To Cart
                  </p>
                  <svg
